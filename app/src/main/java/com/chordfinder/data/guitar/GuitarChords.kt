...
object GuitarChords {
    ...
    private val CANONICAL_POSITIONS = mapOf(
        ...
        "C#" to listOf(-1, 4, 6, 6, 4, 4),  // x46644 (барре на 4 ладу)
        "D#" to listOf(-1, -1, 1, 3, 4, 3), // xx1343
        "F#" to listOf(2, 4, 4, 3, 2, 2),   // 244322 (барре на 2 ладу)
        "G#" to listOf(-1, -1, 6, 8, 8, 8), // xx6888 (барре на 6 ладу)
        "A#" to listOf(-1, 1, 3, 3, 3, 1),  // x13331 (барре на 1 ладу)

        // ===== Бемольные мажорные аккорды =====
        "DB" to listOf(-1, -1, 0, 2, 3, 2), // xx0232 (как D)
        "EB" to listOf(0, 2, 2, 1, 0, 0),   // 022100 (как E)
        "GB" to listOf(3, 2, 0, 0, 0, 3),   // 320003 (как G)
        "AB" to listOf(-1, 0, 2, 2, 2, 0),  // x02220 (как A)
        "BB" to listOf(-1, 1, 3, 3, 3, 1),  // x13331 (как A#)

        // ===== Диезные минорные аккорды =====
        "C#M" to listOf(-1, 4, 6, 6, 5, 4), // x46654 (барре на 4 ладу)
        "D#M" to listOf(-1, -1, 1, 3, 4, 2),// xx1342
        "F#M" to listOf(2, 4, 4, 3, 2, 2),  // 244322 (барре на 2 ладу)
        "G#M" to listOf(-1, -1, 6, 8, 8, 7),// xx6887
        "A#M" to listOf(-1, 1, 3, 3, 3, 1), // x13331 (барре на 1 ладу)

        // ===== Бемольные минорные аккорды =====
        "DM" to listOf(-1, -1, 0, 2, 3, 1), // xx0231 (уже есть)
        "EM" to listOf(0, 2, 2, 0, 0, 0),   // 022000 (уже есть)
        "GM" to listOf(3, 5, 5, 3, 3, 3),   // 355333 (уже есть)

        // ===== 7#5 аккорды (dominant 7th augmented 5th) =====
        "C7#5" to listOf(-1, 3, 2, 3, 2, 0),  // x32320
        "D7#5" to listOf(-1, -1, 0, 2, 2, 2), // xx0222
        "E7#5" to listOf(0, 2, 0, 1, 2, 0),   // 020120
        "F7#5" to listOf(-1, -1, 3, 2, 2, 1), // xx3221
        "G7#5" to listOf(3, 2, 0, 1, 2, 3),   // 320123
        "A7#5" to listOf(-1, 0, 2, 1, 2, 0),  // x02120

        // ===== 7b5 аккорды (dominant 7th diminished 5th) =====
        "C7B5" to listOf(-1, 3, 2, 3, 1, -1), // x3231x (B = b после нормализации)
        "D7B5" to listOf(-1, -1, 0, 2, 1, 2), // xx0212
        "E7B5" to listOf(0, 2, 0, 1, 0, -1),  // 02010x
        "F7B5" to listOf(-1, -1, 3, 2, 1, -1),// xx321x
        "G7B5" to listOf(3, 2, 0, 1, 0, -1),  // 32010x
        "A7B5" to listOf(-1, 0, 2, 0, 1, 0),  // x02010

        // ===== Альтернативные названия 7#5 / 7b5 =====
        "7#5" to listOf(-1, 3, 2, 3, 2, 0),   // x32320 (по умолчанию от C)
        "7+5" to listOf(-1, 3, 2, 3, 2, 0),   // alias для 7#5
        "7B5" to listOf(-1, 3, 2, 3, 1, -1),  // x3231x (7b5)
        "7-5" to listOf(-1, 3, 2, 3, 1, -1),  // alias для 7b5

        // ===== Минорные септаккорды (m7) =====
        "Cm7" to listOf(-1, 3, 1, 3, 1, 0),  // x31310
        "Dm7" to listOf(-1, -1, 0, 2, 1, 1), // xx0211
        "Em7" to listOf(0, 2, 0, 0, 0, 0),   // 020000
        "Fm7" to listOf(-1, -1, 3, 1, 1, 1), // xx3111
        "Gm7" to listOf(3, 5, 3, 5, 3, 3),   // 353533
        "Am7" to listOf(-1, 0, 2, 0, 1, 0),  // x02010

        // ===== Half-diminished (m7b5) =====
        "Cm7B5" to listOf(-1, 3, 1, 3, 1, -1), // x3131x
        "Dm7B5" to listOf(-1, -1, 0, 1, 1, 1), // xx0111
        "Em7B5" to listOf(0, 1, 0, 0, 0, -1),  // 01000x
        "Fm7B5" to listOf(-1, -1, 3, 1, 1, -1),// xx311x
        "Gm7B5" to listOf(3, 5, 3, 5, 3, 3),   // 353533
        "Am7B5" to listOf(-1, 0, 1, 0, 1, -1), // x0101x

        // ===== Мажорные септаккорды с альтерированной квинтой =====
        "CM7#5" to listOf(-1, 3, 2, 0, 2, 0),  // x32020
        "DM7#5" to listOf(-1, -1, 0, 2, 3, 1), // xx0231
        "EM7#5" to listOf(0, 2, 0, 1, 2, 0),   // 020120
        "FM7#5" to listOf(-1, -1, 3, 2, 2, 0), // xx3220
        "GM7#5" to listOf(3, 2, 0, 1, 2, 2),   // 320122
        "AM7#5" to listOf(-1, 0, 2, 1, 2, 0),  // x02120

        // ===== Power chords для всех нот =====
        "B5" to listOf(-1, 2, 4, 4, 0, 0),   // x24400
        "C#5" to listOf(-1, 4, 6, 6, 0, 0),  // x46600
        "D#5" to listOf(-1, -1, 1, 3, 4, 0), // xx1340
        "F#5" to listOf(2, 4, 4, 0, 0, 0),   // 244000
        "G#5" to listOf(-1, -1, 6, 8, 8, 0), // xx6880
        "A#5" to listOf(-1, 1, 3, 3, 0, 0),  // x13300

        // ===== Полные диезные/бемольные аккорды с септаккордами =====
        "C#7" to listOf(-1, 4, 3, 4, 2, 4),  // x43424
        "D#7" to listOf(-1, -1, 1, 3, 2, 3), // xx1323
        "F#7" to listOf(2, 4, 2, 3, 2, 2),   // 242322
        "G#7" to listOf(-1, -1, 6, 8, 7, 8), // xx6878
        "A#7" to listOf(-1, 1, 3, 3, 3, 3),  // x13333

        "DB7" to listOf(-1, -1, 0, 2, 1, 2), // xx0212 (как D7)
        "EB7" to listOf(0, 2, 0, 1, 0, 0),   // 020100 (как E7)
        "GB7" to listOf(3, 2, 0, 0, 0, 1),   // 320001 (как G7)
        "AB7" to listOf(-1, 0, 2, 0, 2, 0),  // x02020 (как A7)
        "BB7" to listOf(-1, 1, 3, 3, 3, 3),  // x13333 (как A#7)

        // ===== Sus2 и Sus4 для диезных/бемольных нот =====
        "C#SUS2" to listOf(-1, 4, 6, 6, 4, 4), // x46644
        "D#SUS2" to listOf(-1, -1, 1, 3, 4, 2),// xx1342
        "F#SUS2" to listOf(2, 4, 4, 0, 0, 0),  // 244000
        "G#SUS2" to listOf(-1, -1, 6, 8, 8, 7),// xx6887
        "A#SUS2" to listOf(-1, 1, 3, 3, 1, 1), // x13311

        "C#SUS4" to listOf(-1, 4, 6, 6, 7, 7), // x46677
        "D#SUS4" to listOf(-1, -1, 1, 3, 4, 4),// xx1344
        "F#SUS4" to listOf(2, 4, 4, 4, 2, 2),  // 244422
        "G#SUS4" to listOf(-1, -1, 6, 8, 8, 8),// xx6888
        "A#SUS4" to listOf(-1, 1, 3, 3, 4, 4), // x13344

        // ===== Add9 для диезных/бемольных нот =====
        "C#ADD9" to listOf(-1, 4, 6, 6, 4, 6), // x46646
        "D#ADD9" to listOf(-1, -1, 1, 3, 4, 3),// xx1343
        "F#ADD9" to listOf(2, 4, 4, 3, 2, 4),  // 244324
        "G#ADD9" to listOf(-1, -1, 6, 8, 8, 8),// xx6888
        "A#ADD9" to listOf(-1, 1, 3, 3, 3, 3), // x13333

        // ===== Diminished для диезных/бемольных нот =====
        "C#DIM" to listOf(-1, 4, 5, 6, 5, 4),  // x45654
        "D#DIM" to listOf(-1, -1, 1, 2, 1, 2), // xx1212
        "F#DIM" to listOf(2, 3, 2, 3, 2, 2),   // 232322
        "G#DIM" to listOf(-1, -1, 6, 7, 7, 7), // xx6777
        "A#DIM" to listOf(-1, 1, 2, 1, 2, 1),  // x12121

        // ===== Augmented для диезных/бемольных нот =====
        "C#AUG" to listOf(-1, 4, 6, 6, 5, 4),  // x46654
        "D#AUG" to listOf(-1, -1, 1, 3, 4, 3), // xx1343
        "F#AUG" to listOf(2, 4, 4, 3, 2, 2),   // 244322
        "G#AUG" to listOf(-1, -1, 6, 8, 8, 7), // xx6887
        "A#AUG" to listOf(-1, 1, 3, 3, 3, 1),  // x13331
    )
